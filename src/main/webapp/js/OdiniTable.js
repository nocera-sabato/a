    ...
    $(document).ready(function () {	
        ...
        while($(".fattura"+c).html() != undefined){
            ...
            //  price= String(price).match(/\d+\.\d+/);
            price= price.match(/\d+\.\d+/);
            var quantita = $(".quantita"+c).html();
            quantita= quantita.match(/\d+/);
            var iva = $(".iva"+c).html();
            iva= iva.match(/\d+/);
            var fattura = $(".fattura"+c).html();
            fattura = fattura.replace(/<\w \w+="\w+\?\w+=\w+\&\w+;\w+=/,"");
            fattura = fattura.replace(/\W.+/,"");
            //calcolo prezzo con iva

            price =  price[0]/100*iva[0]*quantita[0] + price[0]*quantita[0];
              $(".result"+c).html(price);
            c++;
            p++;
        }
        ...
        m=j+1;
            if(($(".fattura"+j).html()==$(".fattura"+m).html())){
                //faccio la somma se e` la stessa fattura
                    price1= $(".result"+j).html();
                    price2 = $(".result"+m).html();
